...
print("\nGive the already allocated resources for each process")
allocated = [[int(i) for i in input(f"Process <{j + 1}> -> ").split()] for j in range(P)]

print("\nGive the maximum resources for each process")
maximum = [[int(i) for i in input(f"Process <{j + 1}> : ").split()] for j in range(P)]
tallocated = [0] * R
for i in range(P):
    for j in range(R):
